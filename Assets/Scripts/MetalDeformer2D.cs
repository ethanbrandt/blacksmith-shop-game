...
namespace ForgingPrototype
{
    ...
    public class MetalDeformer2D : MonoBehaviour
    {
        ...
        public bool TryStrike(Vector2 impactPoint, Vector2 direction, float charge01)
        {
            ...
            if (Vector2.Distance(_vertices[focusIndex], impactPoint) > impactRadius)
                return false;
        ...
        void ApplyLocalOutlineMagnet(Vector2 impactPoint)
        {
            ...
            for (int i = 0; i < _vertices.Count; i++)
            {
                ...
                float pull = Mathf.Pow(Mathf.Clamp01(outlineProximity), magnetFalloff) * magnetStrength * hitWeight * _strikeMagnetMultiplier;
